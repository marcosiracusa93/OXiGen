	...
	unsigned char r_out[SIZE], unsigned char g_out[SIZE], unsigned char b_out[SIZE])
{
	for(int i = WIDTH + 1; i < SIZE - WIDTH - 1; i++) {
		...
		float blur = (
			(
				(0.1 * r_in[i - WIDTH - 1] + 0.6 * g_in[i - WIDTH - 1] + 0.3 * b_in[i - WIDTH - 1]) + 
				(0.1 * r_in[i - 1] + 0.6 * g_in[i - 1] + 0.3 * b_in[i - 1]) + 
				(0.1 * r_in[i + WIDTH - 1] + 0.6 * g_in[i + WIDTH - 1] + 0.3 * b_in[i + WIDTH - 1])
			) / 3 +
			(
				(0.1 * r_in[i - WIDTH] + 0.6 * g_in[i - WIDTH] + 0.3 * b_in[i - WIDTH]) + 
				(0.1 * r_in[i] + 0.6 * g_in[i] + 0.3 * b_in[i]) + 
				(0.1 * r_in[i + WIDTH] + 0.6 * g_in[i + WIDTH] + 0.3 * b_in[i + WIDTH])
			) / 3 +
			(
				(0.1 * r_in[i - WIDTH + 1] + 0.6 * g_in[i - WIDTH + 1] + 0.3 * b_in[i - WIDTH + 1]) + 
		...
		float gray = (0.1 * r_in[i] + 0.6 * g_in[i] + 0.3 * b_in[i]);
	
		float sharpen = fabs(2 * gray - blur);
	
		float ratio = sharpen / (gray + 1);
	
		r_out[i] = fmin(ratio * r_in[i], 255);
		g_out[i] = fmin(ratio * g_in[i], 255);
		b_out[i] = fmin(ratio * b_in[i], 255);
	}
}
